...
def main():
    parser = argparse.ArgumentParser(description="DCQ baseline for few-shot contamination detection.")
    ...
    output = run_dcq_baseline(tokenizer, model, rows, args.temperature, args.seed)

    results_dir = args.results_dir or f"mmlu_baseline_test/{get_dataset_model_tag(args.json_path, args.model_path)}"
    save_jsonl(f"{results_dir}/dcq_results.jsonl", output["rows"])
    save_json(f"{results_dir}/dcq_summary.json", output["summary"])
