...
class IRTModel(AbstractModel):
    ...
    def train(self, train_data: TrainDataset, log_step=None):
        ...
        for ep in range(1, epochs + 1):
            ...
            for cnt, (student_ids, question_ids, _, labels) in enumerate(train_loader):
                ...
                bz_loss.backward()
                optimizer.step()
                total_loss += float(bz_loss.data) * len(labels)
                total_count += len(labels)
                if log_step and cnt % log_step == 0:
                    logging.info('Epoch [{}] Batch [{}]: batch_loss={:.5f}'.format(ep, cnt, float(bz_loss.data)))
            avg_loss = total_loss / total_count if total_count else 0.0
            logging.info('Epoch [{}]: avg_loss={:.5f}'.format(ep, avg_loss))
    ...
    def _loss_function(self, pred, real, reduction='mean'):
        loss = -(real * torch.log(0.0001 + pred) + (1 - real) * torch.log(1.0001 - pred))
        if reduction == 'none':
            return loss
        if reduction == 'sum':
            return loss.sum()
    ...
    def get_item_loss(self, student_ids, question_ids, labels):
        device = self.config['device']
        if not torch.is_tensor(student_ids):
            student_ids = torch.LongTensor(student_ids)
        if not torch.is_tensor(question_ids):
        ...
        with torch.no_grad():
            self.model.eval()
            pred = self.model(student_ids, question_ids).view(-1)
            loss = self._loss_function(pred, labels, reduction='none')
            self.model.train()

        return pred.detach().cpu().numpy(), loss.detach().cpu().numpy()
    ...
    def get_kli(self, student_id, question_id, n, pred_all):
        ...
        # MKLI
        if vegas is None:
            raise ImportError("vegas is required for MKLI integration but is not installed.")
